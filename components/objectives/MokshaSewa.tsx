  ...
  useEffect(() => {
    const fetchInitiatives = async () => {
      try {
        ...
        if (res.data && Array.isArray(res.data.data)) {
          ...
          const fetchedData = res.data.data
            .filter(
              (item: any) =>
                item.status === "Active" &&
                item.objective_catagory === "Moksha Sewa"
            )
            .map((item: any) => {
        ...
        console.error("Error fetching initiatives for Moksha Sewa:", error);
      }
    };
              ...
              >
                <Image
                  src={typeof item.image === "string" ? item.image?.startsWith("http")
                    ? item.image
                    : `${process.env.NEXT_PUBLIC_IMAGE_BASE_URL}${item.image}` : item.image
                  }
                  width={100}
                  height={100}
                  alt={item.title}
